...
#if defined(NDEBUG)
# error "Trinity cannot be compiled without assertions."
#endif
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos)
{
    ...
    // Check the header
    if (!CheckProofOfWork(block.GetPoWHash(), block.nBits, block.GetAlgo()))
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    LogPrintf("InvalidChainFound: invalid block=%s  height=%d  pow_algo=%d  log2_work=%.8g  date=%s\n",
      pindexNew->GetBlockHash().ToString(), pindexNew->nHeight, GetAlgo(chainActive.Tip()->nVersion),
      log(pindexNew->nChainWork.getdouble())/log(2.0), DateTimeStrFormat("%Y-%m-%d %H:%M:%S",
      pindexNew->GetBlockTime()));
    LogPrintf("InvalidChainFound:  current best=%s  height=%d  pow_algo=%d  log2_work=%.8g  date=%s\n",
      chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(), GetAlgo(chainActive.Tip()->nVersion), log(chainActive.Tip()->nChainWork.getdouble())/log(2.0),
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    LogPrintf("UpdateTip: new best=%s  height=%d  pow_algo=%d  log2_work=%.8g  tx=%lu  date=%s progress=%f  cache=%u\n",
      chainActive.Tip()->GetBlockHash().ToString(), chainActive.Height(), GetAlgo(chainActive.Tip()->nVersion),
    ...
    if (!IsInitialBlockDownload() && !fWarned)
    {
        ...
        {
            if ((pindex->nVersion & ~BLOCK_VERSION_ALGO) > CBlock::CURRENT_VERSION)
                ++nUpgraded;
            pindex = pindex->pprev;
        }
        if (nUpgraded > 0)
...
bool CBlockIndex::IsSuperMajority(int minVersion, const CBlockIndex* pstart, unsigned int nRequired)
{
    ...
    {
        if ((pstart->nVersion & ~BLOCK_VERSION_ALGO) >= minVersion)
            ++nFound;
        pstart = pstart->pprev;
    }
